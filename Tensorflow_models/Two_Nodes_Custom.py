"""!@package Two_Nodes_Custom
This is a custom implementation of pythermalcomforts two_nodes_gagge model which 
uses skin temperature as an input so skin temperature can be provided by a ML model

In _gagge_two_nodes_optimized the skin temperature calculation with its differential
is replaced by the skin temperature value input
Also, position default is changed to sitting
zip is used instead of vectorize becaue inputs and outputs are now 33, the max is 32
"""
...
def two_nodes_gagge_ml_skin_temp(
    ...
    * Predicted Percent Satisfied With the Level of Air Movement (PS).
    ...
    tskml : float or list of floats
        Skin temperature predicted by a ML model    
    ...
    if calculate_ce:
        result = _gagge_two_nodes_optimized_return_set_ml_skin_temp(
            tdb,
            tr,
            v,
            met,
    ...
    # Broadcast all variables to identical array shapes
    b_arrays = np.broadcast_arrays(
        tdb, tr, v, met, clo, tskml, vapor_pressure, wme,
        body_surface_area, p_atm, position, calculate_ce,
        max_skin_blood_flow, max_sweating, w_max
    )

    inputs = zip(*b_arrays)

    results = [_gagge_two_nodes_optimized_ml_skin_temp(
        *args) for args in inputs]
    (
        _set,
        e_skin,
        e_rsw,
...
def _gagge_two_nodes_optimized_ml_skin_temp(
    tdb,
    tr,
    v,
    met,
    ...
    alfa = 0.1
    temp_body_neutral = alfa * temp_skin_neutral + \
        (1 - alfa) * temp_core_neutral
    ...
    # evaporative heat loss during comfort
    ...
    if not w_max:  # if the user did not pass a value of w_max
        ...
        if clo > 0:
            # critical skin wettedness clothed
    ...
    # sensible convective heat loss respiration
    ...
    while n_simulation < length_time_simulation:
        ...
        colds = ((-1.0 * sk_sig) > 0) * \
            (-1.0 * sk_sig)  # vasoconstriction signal
    ...
    # PMV Gagge
    pmv_gagge = (0.303 * math.exp(-0.036 * m) + 0.028) * \
        (e_req - e_comfort - e_diff)

    # PMV SET
    dry_set = h_d_s * (t_skin - _set)
    e_req_set = rm - c_res - q_res - dry_set
    pmv_set = (0.303 * math.exp(-0.036 * m) + 0.028) * \
        (e_req_set - e_comfort - e_diff)
...
def _gagge_two_nodes_optimized_return_set_ml_skin_temp(
    tdb,
    tr,
    v,
    met,
    ...
    return _gagge_two_nodes_optimized_ml_skin_temp(
        tdb,
        tr,
        v,
        met,
