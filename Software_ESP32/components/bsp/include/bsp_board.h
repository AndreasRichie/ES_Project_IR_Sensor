...
typedef enum {
  BOARD_BTN_ID_USER = 0,
  BOARD_BTN_ID_BOOT = 1,  // The index is the same as the sequence number of the
                          // board_button_t array
  BOARD_BTN_ID_PREV = 2,
...
typedef struct {
  ...
  gpio_num_t io_num;  // for gpio button, Set GPIO_NUM_NC to indicate this
                      // button is adc button
...
typedef struct {
   ...
   * transmit buffer, then the function will return. Pass portMAX_DELAY for no
   * timeout.
   *
   * @return
   *     - ESP_OK               Success
   *     - ESP_ERR_INVALID_ARG  Parameter error
   */
  esp_err_t (*codec_write)(void *dest, size_t size, size_t *bytes_read,
   ...
   * @param size            Size of data in bytes
   * @param[out] bytes_read Number of bytes read, if timeout, bytes read will be
   * less than the size passed in.
   * @param ticks_to_wait   RX buffer wait timeout in RTOS ticks. If this many
   * ticks pass without bytes becoming available in the DMA receive buffer, then
   * the function will return (note that if data is read from the DMA buffer in
   * pieces, the overall operation may still take longer than this timeout.)
   ...
   *     - ESP_ERR_INVALID_ARG  Parameter error
   */
  esp_err_t (*codec_read)(void *dest, size_t size, size_t *bytes_read,
                          TickType_t ticks_to_wait);
