...
#else
    .rst = EXPANDER_IO_LCD_RESET, .cs = EXPANDER_IO_LCD_CS, .clk = -1, .sdo = -1
#endif
#else
    .rst = -1, .cs = -1, .clk = -1, .sdo = -1
#endif
};
static io_expander_ops_t *io_ops = NULL;

#if LCD_IO_SCHEME != 0  // seeed boards
static esp_err_t _selective_gpio_set_level(gpio_num_t gpio_num,
                                           uint32_t level) {
...
static int init_gpios(struct interface_io *io) {
...
#else
  io_ops = (io_expander_ops_t *)bsp_board_get_description()->io_expander_ops;
  ...
  ESP_LOGD("lcd", "rst: %d, cs: %d, clk: %d, sdo: %d", io->rst, io->cs, io->clk,
           io->sdo);
#endif
#else  // not using io expander
  gpio_config_t io_conf = {};
...
/**
 * @attention
 * Don't write too much data at one time by I2C, otherwise the I2C cmd linked
 * list will occupy a lot of memory. So every time you write some data, you need
