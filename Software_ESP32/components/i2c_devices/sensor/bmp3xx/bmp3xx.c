...
BMP3_INTF_RET_TYPE bmp3_i2c_write(uint8_t reg_addr, const uint8_t *reg_data,
                                  uint32_t len, void *intf_ptr) {
  (void)intf_ptr;
  return i2c_bus_write_bytes(bmp3xx_handle, reg_addr, len,
                             (uint8_t *)(reg_data));
}
...
void bmp3_check_rslt(const char api_name[], int8_t rslt) {
  switch (rslt) {
    ...
    case BMP3_E_COMM_FAIL:
      ESP_LOGE(TAG, "API [%s] Error [%d] : Communication failure", api_name,
               rslt);
      break;
    case BMP3_E_INVALID_LEN:
      ESP_LOGE(TAG, "API [%s] Error [%d] : Incorrect length parameter",
               api_name, rslt);
      break;
    ...
    case BMP3_E_CONFIGURATION_ERR:
      ESP_LOGE(TAG, "API [%s] Error [%d] : Configuration Error", api_name,
               rslt);
      break;
    case BMP3_W_SENSOR_NOT_ENABLED:
      ESP_LOGE(TAG, "API [%s] Error [%d] : Warning when Sensor not enabled",
               api_name, rslt);
      break;
    case BMP3_W_INVALID_FIFO_REQ_FRAME_CNT:
      ESP_LOGE(TAG,
               "API [%s] Error [%d] : Warning when Fifo watermark level is not "
               "in limit",
               api_name, rslt);
      break;
    default:
...
BMP3_INTF_RET_TYPE bmp3_i2c_interface_init(struct bmp3_dev *bmp3,
                                           uint8_t addr) {
...
esp_err_t bmp3xx_init(uint8_t i2c_addr) {
  ...
  ESP_RETURN_ON_FALSE(NULL == bmp3xx_handle, ESP_FAIL, TAG,
                      "bmp3xx already initialized");

  bsp_i2c_add_device(&bmp3xx_handle, i2c_addr);
  ESP_RETURN_ON_FALSE(NULL != bmp3xx_handle, ESP_FAIL, TAG,
                      "add i2c bus device failed");
...
esp_err_t bmp3xx_read_data(float *pressure, float *temperature) {
  ESP_RETURN_ON_FALSE(NULL != bmp3xx_handle, ESP_ERR_INVALID_STATE, TAG,
                      "bmp3xx is not initialized");

  struct bmp3_status status = {{0}, {0}, {0}, 0};
  ...
  if ((rslt == BMP3_OK) && (status.intr.drdy == BMP3_ENABLE)) {
    ...
    *pressure = data.pressure / 100.0;
    ...
    // int)(int32_t)(data.temperature / 100),
