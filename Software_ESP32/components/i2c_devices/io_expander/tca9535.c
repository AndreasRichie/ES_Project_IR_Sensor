...
esp_err_t tca9535_init(uint8_t i2c_addr) {
  ESP_RETURN_ON_FALSE(NULL == tca9535_handle, ESP_FAIL, TAG,
                      "tca9535 already initialized");
...
esp_err_t tca9535_multi_write_start(void) {
  ESP_RETURN_ON_FALSE(NULL != tca9535_handle, ESP_ERR_INVALID_STATE, TAG,
                      "tca9535 is not initialized");
  ESP_RETURN_ON_FALSE(NULL == g_i2c_cmd, ESP_ERR_INVALID_STATE, TAG,
                      "tca9535_multi_write_end is not be called");
  g_i2c_cmd = i2c_cmd_link_create();
  uint8_t addr = i2c_bus_device_get_address(tca9535_handle);

  i2c_master_start(g_i2c_cmd);
  i2c_master_write_byte(g_i2c_cmd, (addr << 1) | I2C_MASTER_WRITE,
                        I2C_ACK_CHECK_EN);
  return i2c_master_write_byte(g_i2c_cmd, tca9535_OUTPUT_PORT_REG,
                               I2C_ACK_CHECK_EN);
}
...
esp_err_t tca9535_multi_write_new_level(int pin, bool new_level) {
  ESP_RETURN_ON_FALSE(NULL != tca9535_handle, ESP_ERR_INVALID_STATE, TAG,
                      "tca9535 is not initialized");
  ESP_RETURN_ON_FALSE(NULL != g_i2c_cmd, ESP_ERR_INVALID_STATE, TAG,
                      "tca9535_multi_write_start is not be called");
  if (new_level) {
    output_reg |= BIT(pin);
  } else {
    output_reg &= ~BIT(pin);
  }
  return i2c_master_write(g_i2c_cmd, (uint8_t *)(&output_reg), 2,
                          I2C_ACK_CHECK_EN);
}
